    ...
    private async Task<Task> ConfigureMainMenuAsync(MenuConfigurationContext context)
    {
        ...
        if (await context.IsGrantedAsync(BookStorePermissions.Book.Default))
        {
            context.Menu.GetAdministration().AddItem(
                new ApplicationMenuItem(BookStoreMenus.Book, l["Menu:Book"], "/books")
            );
        }
        if (await context.IsGrantedAsync(BookStorePermissions.Category.Default))
        {
            context.Menu.GetAdministration().AddItem(
                new ApplicationMenuItem(BookStoreMenus.Category, l["Menu:Category"], "/Categories/Category")
            );
        }
        ...
        {
            context.Menu.GetAdministration().AddItem(
                new ApplicationMenuItem(BookStoreMenus.Book, l["Menu:Book"], "/Books/Book")
            );
        }
    }
